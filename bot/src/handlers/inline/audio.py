...
async def inline_handler(query: types.InlineQuery):
    ...
    if not user or not await db_client.check_auth(query.from_user.id):
        ...
        try:
            track = await get_now_playing_track(user.api_key, user.id)
            results = [add_text_track(track)]
            await query.answer(results=results, cache_time=0)
        except ValueError:
            await query.answer(
                results=[
                    await add_text_result(
                        Icons.ERROR, "inline-error", "error-fetching", lang
                    )
                ],
                cache_time=0,
            )
        ...
        try:
            track = await get_now_playing_track(
                user.api_key, user.id, query.query.strip()
            )
    ...
    elif query.query.strip().lower() == "audio" or query.query.strip() == "":
        try:
            track = await get_now_playing_track(user.api_key, user.id)
            results = [add_audio_track(track)]
            await query.answer(results=results, cache_time=0)
        except ValueError as e:
            logger.warning(f"Error fetching track for inline audio: {e}")
            await query.answer(
                results=[
                    await add_text_result(
                        Icons.ERROR, "inline-error", "error-fetching", lang
